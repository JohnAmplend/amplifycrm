...
Deno.serve(async (req) => {
  try {
    ...
    const { objectType, recordIds, stage } = await req.json();

    if (!objectType || !recordIds || !stage) {
    ...
    const stageField = objectType === 'Deal' ? 'deal_stage' : 'lifecycle_stage';

    // Create bulk operation record
    const operation = await base44.asServiceRole.entities.Bulk_Operations.create({
      operation_type: 'Update Stage',
      object_type: objectType,
      total_records: recordIds.length,
      successful_records: 0,
      failed_records: 0,
      performed_by: user.email,
      operation_metadata: { stage, field: stageField },
      can_undo: true
    });
    ...
    for (const recordId of recordIds) {
      try {
        ...
        snapshot.push({
          record_id: recordId,
          old_data: { [stageField]: record[stageField] },
          new_data: { [stageField]: stage }
        });
