...
Deno.serve(async (req) => {
  try {
    ...
    const connections = await base44.asServiceRole.entities.GmailAccount.list();
    if (!connections || connections.length === 0) {
      return Response.json({ message: "No Gmail connections found" });
    }
      ...
      try {
        ...
        if (tokenExpiry < new Date(Date.now() + 60000)) {
          ...
          if (!tokens.error) {
            ...
            await base44.asServiceRole.entities.GmailAccount.update(conn.id, {
              access_token: tokens.access_token,
              expires_at: new Date(Date.now() + tokens.expires_in * 1000).toISOString(),
            });
          } else {
            console.error(`Token refresh failed for ${conn.user_email}:`, tokens.error);
            continue;
          }
        }
        ...
        const existing = await base44.asServiceRole.entities.EmailMessage.filter({ user_email: conn.user_email });
        ...
        console.error(`Error syncing ${conn.user_email}:`, connErr.message);
      }
    }

    return Response.json({ message: `Auto-sync complete. ${totalSynced} new emails synced.` });
  } catch (error) {
