...
export default function EmailInbox() {
  ...
  const syncEmails = async () => {
    ...
    setSyncError(null);
    try {
      const res = await base44.functions.invoke('gmailSyncEmails', {});
  ...
  const sendEmail = async () => {
    ...
    setSending(true);
    try {
      const res = await base44.functions.invoke('gmailSendEmail', {
        to: compose.to,
        subject: compose.subject,
        body: compose.body,
        replyToMessageId: replyTo?.gmail_message_id || null,
      });
      if (res.data?.error) throw new Error(res.data.error);
      setComposing(false);
